...
VALUES 
    ...
    ('remember_me_duration_days', '30', 'Number of days a "remember me" session remains valid'),
    ('default_pagination_limit', '10', 'Default number of items per page in lists')
ON CONFLICT (setting_key) DO NOTHING;

-- Create countries table if not exists (it seems missing)
...
-- Re-add country_id to clubs if missing (it WAS missing in the \d output)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clubs' AND column_name = 'country_id') THEN
        ALTER TABLE clubs ADD COLUMN country_id UUID REFERENCES countries(id) ON DELETE RESTRICT;
