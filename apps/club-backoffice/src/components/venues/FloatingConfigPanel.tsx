...
import { ChevronDown, ChevronRight, Plus, Trash2, Copy, Edit2 } from 'lucide-react';
...
const FloatingConfigPanel: React.FC<FloatingConfigPanelProps> = ({
    selectedStand,
    onUpdateStand,
    onDeleteStand,
    onAddSector,
    ...
    if (!selectedStand) {
        return (
            <div className="stadium-config-panel stadium-glass stadium-animate-slide-in">
                <div style={{
                    display: 'flex',
    ...
    return (
        <div className="stadium-config-panel stadium-glass-intense stadium-animate-slide-in">
            {/* Stand Info Section */}
                    ...
                    <div className="stadium-animate-fade-in">
                        <div style={{ marginBottom: 12 }}>
                            ...
                            <input
                                ...
                                onChange={(e) => onUpdateStand(selectedStand.id, { name: e.target.value })}
                        ...
                        <div style={{ marginBottom: 12 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                {['#00d4ff', '#00ff88', '#ffaa00', '#ff4466', '#a855f7'].map(color => (
                                    <button
                                        key={color}
                                        onClick={() => onUpdateStand(selectedStand.id, { color })}
                                        style={{
                                            width: 36,
                                            height: 36,
                                            borderRadius: 8,
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <span className="stadium-badge stadium-badge-success">
                                {selectedStand.capacity || 0} lugares
                            </span>
                        </div>
                    </div>
                )}
                                ...
                                <div className="stadium-card-header">
                                                    ...
                                                    <div key={sector.id} style={{
                                                        ...
                                                        <div style={{ display: 'flex', gap: 4 }}>
                                                            <button
                                                                className="stadium-btn stadium-btn-ghost"
                                                                style={{ padding: 4 }}
                                                                onClick={() => onEditSector(selectedStand.id, floor.id, sector.id)}
                                                                title="Editar Setor"
                                                            >
                                                                <Edit2 size={12} />
                                                            </button>
                                                            <button
                                                                ...
                                                                onClick={() => onRemoveSector(selectedStand.id, floor.id, sector.id)}
                                                                title="Remover Setor"
                                                            >
                                                                <Trash2 size={12} />
                                                            </button>
                                    ...
                                    <div style={{ display: 'flex', gap: 4 }}>
                                        <button
                                            ...
                                            onClick={() => onAddSector(selectedStand.id, floor.id)}
                                        >
                                            <Plus size={16} />
                                        </button>
                                        <button
            ...
            <div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                <button
                    ...
                    onClick={() => onDeleteStand(selectedStand.id)}
                >
                    <Trash2 size={16} />
                    Remover Bancada
                </button>
